...
import { createReservation, getFormData, getGateHolidays, getGateBlocksForDate } from "@/lib/actions/reservations";
...
export function ReservationFormDialog({
  ...
  // Load holidays when gate or date month changes
  useEffect(() => {
  ...
  }, [gateId, date.slice(0, 7)]); // re-fetch when month changes

  // Load gate blocks when gate or date changes
  useEffect(() => {
  ...
  const rawTimeSlots = (!isAdmin && holidayName) ? [] : getAvailableSlots(selectedGate, parsedDate, isAdmin);
